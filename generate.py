...
parser.add_argument('--next', default="%ether1", metavar = "INTERFACE OR IP",
                    help='next hop for where non-China IP address, this is usually the tunnel interface')
parser.add_argument('--chn_list', default=['dependency/china_ip_list.txt', 'dependency/china.txt', 'dependency/chnroutes.txt'], nargs='*',
...
def dump_rsc(lst, f):
    f.write('/ip/route/remove [find where distance=10]\n')
...
def dump_rds_inner(lst, f):
    for n in lst:
        ...
        if len(n.child) > 0:
            ...
            f.write('/ip/route/add distance=10 dst-address=%s gateway=%s routing-table=noncn\n' % (n.cidr, args.next))
